        ...
        if first_level.is_dir():          
            ...
            for second_level in first_level.glob('*'):
                if second_level.is_dir():
                    ...
                    # Run the pipeline specifying the number of images to generate
                    p.sample(200)
            ...
            for second_level in first_level.glob('*'):
                if second_level.is_dir():
                    ...
                    for file in second_level.glob('*'):
                            if file.is_file():
                                # normalise images (resize 40x40, grayscale filter, blur filter, rename to convention (#_Shape_#shapes))
